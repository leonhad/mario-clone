...
namespace dat {
    ...
    inline void CalcCrc32(char byte, uint32_t &crc32)
    {
        crc32 = ((crc32) >> 8) ^ crcTable[(byte) ^ ((crc32) & 0x000000FF)];
    }

    bool Crc::stringCrc32(uint8_t *string, uint32_t &crc32)
    {
        bool errorCode = true;
    ...
    bool Crc::crc32(uint8_t *string, uint32_t tamanho, uint32_t &crc32)
    {
        bool errorCode = true;
        uint32_t loop;
